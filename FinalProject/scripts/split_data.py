...
def split_credit_data():
    raw_source = os.path.join(os.path.dirname(__file__), "..", "..", "Lab2", "data", "credit_default.csv")
    ...
    df_older = df[df["AGE"] >= 30].sample(frac=1.0, random_state=42).reset_index(drop=True)
    df_younger = df[df["AGE"] < 30].sample(frac=1.0, random_state=42).reset_index(drop=True)

    print(f"Older cohort (AGE >= 30): {len(df_older)} records (Mean age: {df_older['AGE'].mean():.1f})")
    print(f"Younger cohort (AGE < 30): {len(df_younger)} records (Mean age: {df_younger['AGE'].mean():.1f})")
    ...
    print(f"Saved {len(baseline_df)} records to {baseline_path} (Mean age: {baseline_df['AGE'].mean():.1f})")
    ...
    print(f"Saved {len(normal_stream_df)} records to {normal_path} (Mean age: {normal_stream_df['AGE'].mean():.1f})")
    ...
    print(f"Saved {len(drifted_features)} records to {drifted_path} (Mean age: {drifted_features['AGE'].mean():.1f})")
